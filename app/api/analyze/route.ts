...
// In a real multi-instance deployment, move this to a shared store (Redis, Upstash, etc).
const rateState = new Map<string, number[]>();
function isRateLimited(ip: string, nowMs = Date.now()): { limited: boolean; retryAfterSec: number } {
  ...
  const over = times.length > LIMITS.requestsPerMinutePerIp;
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json({
      ...
      source: "fallback",
      notice:
        "Gemini API 키가 설정되어 있지 않아 AI 분석을 수행하지 못했습니다. (.env 파일에 GEMINI_API_KEY를 추가하세요)",
    });
  }

  try {
    const aiInfo = await analyzeWithGemini(query, apiKey);
    return NextResponse.json({ info: aiInfo, source: "ai" });
  } catch (err) {
